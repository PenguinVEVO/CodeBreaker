...
namespace DarkTreeFPS
{
    ...
    public class Weapon : MonoBehaviour
    {
        ...
        {
            var _npcs = FindObjectsOfType<NPCVision>();
            
            foreach(var _npc in _npcs)
            {
                //onShot.AddListener(() => _npc.TriggerPlayerShotVisibility());
            }
        }
        ...
        void Update()
        {
            ...
            if (Input.GetKey(input.Fire) &&!InputManager.useMobileInput && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType != WeaponType.RocketLauncher  && weaponType != WeaponType.Pistol && !InventoryManager.showInventory && fireMode == FireMode.automatic)  //Statement to restrict auto-fire for pistol weapon type. Riffle and others are automatic
            {
                Fire();
            }
            else if (Input.GetKeyDown(input.Fire) && !InputManager.useMobileInput && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType != WeaponType.RocketLauncher && (weaponType == WeaponType.Pistol || fireMode == FireMode.single) && !InventoryManager.showInventory)
            {
                Fire();
            }
            else if(Input.GetKeyDown(input.Fire) && !InputManager.useMobileInput && !FPSController.isSwimming && Time.timeScale != 0 && !PlayerStats.isPlayerDead && weaponType == WeaponType.RocketLauncher && !InventoryManager.showInventory)
            {
                FireRocketLauncher();
            }
            ...
            if (weaponType != WeaponType.Melee && weaponType != WeaponType.Grenade)
            {
                ...
                if (currentItem != null && ( Input.GetKeyDown(input.Reload) || currentItem.stackSize == 0))
                {
                    if (weaponType != WeaponType.Shotgun)
                    {
                        if (!reloading && !controller.isClimbing && currentItem.stackSize < maxAmmo)
                            ReloadBegin();
                    }
                    else
                    {
                ...
                {
                    if (Input.GetKey(input.Aim) && !controller.PlayerCloseToWall())
                    {
                        setAim = true;
            ...
            if (weaponType == WeaponType.Shotgun)
            {
                ...
                if (Input.GetKeyDown(input.Fire) && reloading)
                {
                    animator.SetBool("Reloading", false);
                }
            ...
            if (weaponType != WeaponType.Melee && weaponType != WeaponType.Grenade && !reloading)
            {
                if (Time.time > nextFireTime && !reloading && canShot && !controller.isClimbing) //Allow fire statement
                {
                    if (currentItem.stackSize > 0)
                    {
                        ...
                        if (CoverList.sceneActiveNPC != null)
                        {
        ...
        public void MeleeHit()
        {
            ...
            if (Physics.Raycast(mainCamera.transform.position, mainCamera.transform.forward, out hit, meleeAttackDistance))
            {
                ...
                if (hit.collider.GetComponent<PlayerHitTarget>())
                    hit.collider.GetComponent<PlayerHitTarget>().npc.GetHit(meleeDamagePoints, transform);
        ...
        private void UpdateAmmoText()
        {
            ...
            if(weaponType != WeaponType.Grenade)
                if(currentItem != null)
            ammoText.text = string.Format("{0}|{1}\n Mode:{2}", currentItem.stackSize, CalculateTotalAmmo(), mode);
        }
